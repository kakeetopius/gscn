...
func TestTargetsFromString(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "ipv4 cidr",
			input:   []string{"10.1.1.0/24"},
			wantLen: 1,
		},
		{
			name:    "ipv6 cidr",
			input:   []string{"2001:abcd::1/64"},
			wantLen: 1,
		},
		{
		...
		// --- ranges which are valid ---
		{
			...
			input:   []string{"10.1.1.1-2"},
			wantLen: 2,
		},
		{
			name:    "single-element range start == end",
			input:   []string{"10.1.1.5-5"},
			wantLen: 1,
		},
		{
			name:    "range at octet boundary 254-255",
			input:   []string{"10.1.1.254-255"},
			wantLen: 2,
		},
		...
		// --- Valid: comma-separated mixed ---
		{
			...
			input:   []string{"10.1.1.0/24", "10.2.2.2"},
			wantLen: 2,
		},
		{
			name:    "cidr and range",
			input:   []string{"10.1.1.0/24", "10.3.3.1-3"},
			wantLen: 4,
		},
		{
			name:    "all three types",
			input:   []string{"10.1.1.1/24", "10.1.1.1", "10.1.1.1-2"},
			wantLen: 3,
		},
		{
			name:    "spaces around comma",
			input:   []string{"10.1.1.0/24", "2001:abcd::1/64"},
			wantLen: 2,
		},
		{
...

		{
			...
			input:   []string{"10.1.1.0/24", "10.1.1.0/24"},
			wantLen: 1,
		},
		{
			name:    "duplicate single ips",
			input:   []string{"10.1.1.1", "10.1.1.1"},
			wantLen: 1,
		},
		{
			name:    "three copies deduplicated",
			input:   []string{"10.1.1.1", "10.1.1.1", "10.1.1.1"},
			wantLen: 1,
		},
		...
		},
		{
			...
			input:   []string{"10.1.1.1-abc"},
			wantErr: true,
		},
		{
			name:    "range end exceeds 255",
			input:   []string{"10.1.1.1-300"},
			wantErr: true,
		},
		{
			name:    "one valid one invalid",
			input:   []string{"10.1.1.1", "garbage"},
			wantErr: true,
		},
		{
			name:    "ip with extra octets",
			input:   []string{"10.1.1.1.1"},
			wantErr: true,
		},
		{
...
func TestTargetsFromStringWithDNSLookup(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:     []string{"10.1.1.1-3"},
			wantLen:   3,
			wantHosts: 0,
		},
		{
			name:      "mixed ip types no dns",
			input:     []string{"10.1.1.0/24", "10.2.2.2", "10.3.3.1-2"},
			wantLen:   4,
			wantHosts: 0,
		},
		...
		},
		{
			...
			input:          []string{"bing.com", "google.com"},
			wantLen:        2,
			wantHosts:      2,
		...
		// --- Mixed IP and domain ---
		{
			...
			input:          []string{"10.1.1.0/24", "bing.com"},
			wantLen:        2,
			wantHosts:      1,
			checkHostnames: []string{"bing.com"},
		},
		{
			...
			input:          []string{"10.1.1.1/24", "10.1.1.1", "bing.com", "10.1.1.1-2", "google.com"},
			wantLen:        5,
			wantHosts:      2,
			checkHostnames: []string{"bing.com", "google.com"},
		},
		{
			...
			input:          []string{"8.8.8.8", "google.com"},
			wantLen:        2,
			wantHosts:      1,
			checkHostnames: []string{"google.com"},
		},
		...
		// --- Deduplication ---
		{
			...
			input:          []string{"bing.com", "bing.com"},
			wantLen:        1,
			wantHosts:      1,
			checkHostnames: []string{"bing.com"},
		},
		...
		},
		{
			...
			input:   []string{"this.domain.does.not.exist.invalid"},
			wantErr: true,
		},
		{
			name:    "one valid ip one bad domain",
			input:   []string{"10.1.1.1", "this.domain.does.not.exist.invalid"},
			wantErr: true,
		},
		{
			name:    "one valid domain one bad domain",
			input:   []string{"google.com", "this.domain.does.not.exist.invalid"},
			wantErr: true,
		},
		{
			name:    "invalid ip with valid domain",
			input:   []string{"bing.com", "10.1.1.1/33"},
			wantErr: true,
		},
		{
